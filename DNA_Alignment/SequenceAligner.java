...
public class SequenceAligner {
    ...
    private void fillCache() {
        ...
        for (int i = 1; i <= n; i++) {
            for (int j = 1; j <= m; j++) {
                ...
                int delete = cache[i - 1][j].getScore() + judge.getGapCost();
                int insert = cache[i][j - 1].getScore() + judge.getGapCost();
                int match = cache[i - 1][j - 1].getScore() + judge.score(x.charAt(i - 1), y.charAt(j - 1));
                Direction d;
    ...
    private void traceback() {
        ...
        while (i > 0 || j > 0) {
            Result MID = cache[i][j];
            Direction d = MID.getParent();
